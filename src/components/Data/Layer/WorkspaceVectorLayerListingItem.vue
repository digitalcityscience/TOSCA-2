<template>
  <div v-if="props.item" class="first:pt-0 pt-1">
    <Card>
      ...
      <template
        #subtitle
        v-if="layerDetail && layerDetail?.featureType.abstract?.length > 0"
      >
        <span class="line-clamp-3 hover:line-clamp-none xl:line-clamp-none">{{
      ...
      <template #content v-if="layerDetail">
        <div class="grid grid-cols-4 w-full pt-1">
          <span
            class="font-bold lg:col-span-2 2xl:col-span-2 3xl:col-span-2 4xl:col-span-1 self-center"
            >Keywords:</span
          >
          <span
            class="lg:col-span-2 2xl:col-span-2 3xl:col-span-2 4xl:col-span-3 pl-1"
          >
            <Tag
              class="mb-1 mr-1 last:mr-0"
              severity="primary"
              v-for="(keyword, index) in layerDetail.featureType.keywords
                .string"
              :key="index"
              :value="keyword"
            ></Tag>
  ...
  <div v-else class="first:pt-0 pt-1 w-full">
    <InlineMessage class="w-full" severity="info"
      >No information about layer.</InlineMessage
    >
  </div>
</template>
...
const cleanLayerName = computed(() => {
    return layerDetail.value?.featureType.title != null
        ? layerDetail.value?.featureType.title.replaceAll("_", " ")
        : props.item.name.replaceAll("_", " ");
});
const geoserver = useGeoserverStore();
const layerDetail = ref<GeoServerVectorTypeLayerDetail>();
geoserver
    .getLayerDetail(props.layerInformation.resource.href)
    .then((detail) => {
...
const dataType = computed(() => {
    if (!isNullOrEmpty(layerDetail.value)) {
        const feature = layerDetail.value!.featureType.attributes.attribute.filter(
            (att) => {
                return att.name.includes("geom");
            }
        );
        return feature.length > 0
            ? sanitizeDataType(feature[0].binding.split(".").slice(-1)[0])
            : "";
    } else {
        return "";
    }
            ...
            .then(() => {
                if (!isNullOrEmpty(dataType) && !isNullOrEmpty(layerDetail.value)) {
                    const layerParams: LayerParams = {
                        ...
                        layerStyle: !isNullOrEmpty(props.layerStyling)
                            ? { ...props.layerStyling }
                            : undefined,
