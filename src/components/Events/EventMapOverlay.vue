...
async function waitForMapStyle(): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
        mapStore.map.once("load", resolve);
    });
}
...
function handleMapClick(event: MapMouseEvent): void {
    ...
    if (isClusterFeature(feature)) {
        showClusterPopup(feature, event).catch(showError);
        return;
    }
...
function showEventPopup(eventProperties: EventMapProperties, event: MapMouseEvent): void {
    popup?.remove();
    ...
    render(h(EventMapPopup, {
        ...
        onOpenDetails: () => {
            ...
            router.push({
                name: "event-detail",
                params: { eventId: eventProperties.id },
            }).catch(() => {});
        },
    }), popupContainer);
    popup = new maplibre.Popup({
        className: "tosca-map-popup tosca-event-map-popup",
        maxWidth: "none",
        closeButton: true,
        offset: 12,
    })
...
async function showClusterPopup(
    feature: maplibre.MapGeoJSONFeature,
    event: MapMouseEvent
): Promise<void> {
    const source = mapStore.map.getSource(EVENT_SOURCE_ID) as GeoJSONSource | undefined;
