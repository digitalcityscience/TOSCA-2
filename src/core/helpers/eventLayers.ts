...
import { type EventDetail, type EventLayerLink } from "@store/events";
...
export interface LoadEventLayersOptions {
    onLayerError?: (error: unknown, layer: EventLayerLink) => void;
}

function getOrderedRenderableLayers(event: EventDetail): EventLayerLink[] {
    return [...event.layers]
...
export async function loadEventLayersOnMap(
    event: EventDetail,
    geoserverStore: GeoserverStore,
    mapStore: EventMapStore,
    options: LoadEventLayersOptions = {}
): Promise<number> {
    ...
    for (const item of getOrderedRenderableLayers(event)) {
